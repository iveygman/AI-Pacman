...
class CornersProblem(search.SearchProblem):
  ...
  def getStartState(self):
    " Returns starting position and whether the four corners were visited"
  ...
  def isGoalState(self, state):
    ...
    self._visitedlist.append(state[0]); # note that we've been here
    return True;
  ...
  def getSuccessors(self, state):
    ...
    # find which corner we are on, if any
    if (state[0] in self.corners):
        for i,corner in enumerate(self.corners):
            if (state[0] == corner):    # 
                corner_list[i] = True;
    corner_tuple = tuple(corner_list);  # repack them
    
    for a in [Directions.NORTH,Directions.SOUTH,Directions.EAST,Directions.WEST]:
      x,y = state[0];
      dx,dy = Actions.directionToVector(a);
      nextx, nexty = int(x + dx), int(y + dy);
      if (self.walls[nextx][nexty] is False):   # doesn't hit the wall
        successors.append( ( ((nextx,nexty),corner_tuple), a,1 ) );
      
    self._expanded += 1;
    if (state[0] not in self._visited):
        self._visited[state[0]] = True; # mark it visited
...
def euclidianDistance( xy1, xy2 ):
    return pow(pow(xy1[0]-xy2[0],2) + pow(xy1[1]-xy2[1],2),0.5);
...
def cornersHeuristic(state, problem):
  ...
  for n,k in enumerate(problem.corners):
    if not state[1][n]:     # don't count states that are explored
        cost = util.manhattanDistance(state[0],k);
        
        # now add a penalty for the walls in the way
        added_cost = [0,0];
        y = k[1];
        for x in range(state[0][0],k[0]):
            if walls[x][y] is True:
                added_cost[0] += wall_penalty;
        x = k[0];
        for y in range(state[0][1],k[1]):
            if walls[x][y] is True:
                added_cost[0] += wall_penalty;
        ...
        d.append(cost+sum(added_cost)/2);
